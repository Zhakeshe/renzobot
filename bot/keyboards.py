...
def payment_methods_kb(lang: str = 'kz'):
    builder = InlineKeyboardBuilder()
    if lang == 'kz':
        builder.button(text="💳 Kaspi (Аударма)", callback_data="topup_kaspi", style="primary")
        builder.button(text="🤖 CryptoBot", callback_data="topup_cryptobot", style="primary")
        builder.button(text="🌟 Telegram Stars", callback_data="topup_stars", style="success")
        builder.button(text="🔙 Артқа", callback_data="profile", style="danger")
    else:
        builder.button(text="💳 Kaspi (Перевод)", callback_data="topup_kaspi", style="primary")
        builder.button(text="🤖 CryptoBot", callback_data="topup_cryptobot", style="primary")
        builder.button(text="🌟 Telegram Stars", callback_data="topup_stars", style="success")
        builder.button(text="🔙 Назад", callback_data="profile", style="danger")
    builder.adjust(1)
    return builder.as_markup()

def payment_paid_kb(req_id: int, lang: str = 'kz'):
    builder = InlineKeyboardBuilder()
    text = "✅ Төледім / Оплатил" if lang == 'kz' else "✅ Оплатил"
    builder.button(text=text, callback_data=f"pay_confirm_{req_id}", style="success")
    back_text = "❌ Бас тарту / Отмена" if lang == 'kz' else "❌ Отмена"
    builder.button(text=back_text, callback_data="topup", style="danger")
    builder.adjust(1)
    return builder.as_markup()

def admin_payment_request_kb(req_id: int):
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Растау", callback_data=f"adm_pay_approve_{req_id}", style="success")
    builder.button(text="❌ Бас тарту", callback_data=f"adm_pay_reject_{req_id}", style="danger")
    builder.adjust(2)
    return builder.as_markup()
...
def main_menu_kb(is_admin: bool = False, lang: str = 'kz'):
    builder = InlineKeyboardBuilder()
    if lang == 'kz':
        builder.button(text="🌟 Stars сатып алу", callback_data="buy_stars", style="success")
        builder.button(text="💰 Stars сату", callback_data="sell_stars", style="primary")
        builder.button(text="🏠 NFT жалдау", callback_data="rent_nft", style="primary")
        builder.button(text="📦 NFT сатып алу", callback_data="buy_nft", style="primary")
        builder.button(text="🎁 Кәдімгі сыйлық", callback_data="buy_gift", style="primary")
        builder.button(text="💎 TON сатып алу", callback_data="buy_ton", style="primary")
        builder.button(text="✨ Премиум", callback_data="buy_premium", style="primary")
        builder.button(text="💳 Толтыру", callback_data="topup", style="success")
        builder.button(text="👤 Профиль", callback_data="profile", style="primary")
        builder.button(text="🛠 Қолдау", callback_data="support", style="primary")
        builder.button(text="🧮 Калькулятор", callback_data="calculator", style="primary")
        builder.button(text="👥 Реферал жүйесі", callback_data="referral", style="primary")
        builder.button(text="🏆 Топ клиенттер", callback_data="top_clients", style="primary")
        builder.button(text="💕 Пікірлер", url="https://t.me/reviews")
        builder.button(text="🎁 Нұсқаулық", url="https://t.me/instruction")
    else:
        builder.button(text="🌟 Купить звёзды", callback_data="buy_stars", style="success")
        builder.button(text="💰 Продать звёзды", callback_data="sell_stars", style="primary")
        builder.button(text="🏠 Аренда NFT", callback_data="rent_nft", style="primary")
        builder.button(text="📦 Купить NFT.", callback_data="buy_nft", style="primary")
        builder.button(text="🎁 Купить обычный подарок.", callback_data="buy_gift", style="primary")
        builder.button(text="💎 Купить TON", callback_data="buy_ton", style="primary")
        builder.button(text="✨ Премиум", callback_data="buy_premium", style="primary")
        ...
        builder.button(text="🧮 Калькулятор", callback_data="calculator", style="primary")
        builder.button(text="👥 Реферальная система", callback_data="referral", style="primary")
        builder.button(text="🏆 Топ клиентов", callback_data="top_clients", style="primary")
        builder.button(text="💕 Отзывы", url="https://t.me/reviews")
        builder.button(text="🎁 Инструкция", url="https://t.me/instruction")
        
    if is_admin:
        builder.button(text="📊 Админ панель", callback_data="admin_panel", style="danger")
